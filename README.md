...
- The **File** menu's single **Export…** item opens a modal with the actual export settings (format, active-frame-vs-whole-animation, raster scale) rather than a menu crowded with dropdowns; **Import Image…** and **Reference Image…** live there too, each opening its own settings modal
...
- Export to SVG, PNG, or WebP (at 1x/4x/8x/16x) via the Export modal, and `.pixelyph` project save/load — exported SVG layers get a CSS-selectable `id` derived from the layer name
...
- Color inputs throughout are one consistent square swatch that opens a popover — a hex text field (accepts 3/4/6/8-digit shorthand) plus R/G/B/A sliders with editable number boxes, and a "Color Picker" button opening a fully custom, app-styled saturation/value + hue picker (not the native, unstyleable `<input type="color">` popup) with a real screen-sampling eyedropper (the `EyeDropper` API) — every opacity/alpha slider (layer opacity, color alpha, reference-image opacity) pairs with a directly-editable percentage box
