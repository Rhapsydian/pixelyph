...
Early development, but broadly functional. Draw mode (Pixel and Shape tiers, multi-layer, frame-based animation, animated GIF/PNG export), Glyph mode (a unified glyph model — no more locked-at-creation character-font/icon-font split, a glyph can freely mix a real typed character and/or a name), project management (startup screen, new-project wizard, autosave recovery), font compilation, an in-app User Manual, the Electron desktop shell (with a CI-built Windows installer available from the Help menu), a full visual design pass (real app layout, token-based dark theme, viewport minimap, resizable panels), and a functional Palette/Layers/Style review (a shared, exportable palette covering colors/gradients/saved styles, per-layer thumbnails, a custom-built HSV color picker, a Transform menu with a Canvas/Layer/Shape target picker, and on-canvas gradient handles) are all implemented — see "Features" below. SVG pattern fills were built and then deliberately removed, pending a real authoring UI; auto-update for the Windows build is scoped but deferred behind an unresolved local packaging issue (see `BACKLOG.md`). Two optional per-glyph color layers exist in the model as groundwork for a future two-/three-color icon-font export, with no editing UI or export wiring yet (see `BACKLOG.md`). The in-app User Manual was rewritten as step-by-step, task-oriented walkthroughs for brand-new users, with any prior-version comparison language removed; two bundled example projects are now reachable from File → Sample Projects. In Shape tier, erasing or cutting a shape down to zero pixels now leaves it as a persistent empty shape instead of deleting it, and internal Copy/Cut → Paste now merges back into an existing shape (the active shape for a single-shape source, or each piece's own original shape for a same-layer multi-shape source) instead of always creating a new one — external image paste is unaffected. Pixelyph is playable on itch.io at [rhapsydian.itch.io/pixelyph](https://rhapsydian.itch.io/pixelyph) — the manual zip-upload path 404'd on subdirectories (an itch.io-side bug, not a Pixelyph build issue), so deploys now go through itch's own `butler` CLI instead, which uploads files directly and sidesteps the bug entirely (see `BACKLOG.md`). The GitHub Pages build now serves from the `pixelyph.com` custom domain root instead of the `rhapsydian.github.io/pixelyph` project-page subpath (see `BACKLOG.md`). The glyph-set JSON manifest export now carries a font-level meta block and full per-glyph metrics (advance width, offset, raw width/height), keyed by codepoint rather than the regenerable CSS-class slug, and is independently exportable from the icon-font CSS — done to unblock a separate in-development project (Glyphrogue) that imports Pixelyph's compiled fonts and this manifest as a font source.
...
**Font compilation** turns a GlyphSet into a real, installable font (OTF/WOFF) via [opentype.js](https://github.com/opentypejs/opentype.js), with independently optional icon-font CSS and a JSON manifest (font meta plus full per-glyph metrics, keyed by codepoint) for downstream tooling, and a self-contained demo HTML file bundled into every export — its specimen preview mirrors the in-app one (insert-at-cursor glyph swatches, the same hybrid per-character color model), rendered through the real embedded font instead of an approximation.

**Project management** greets you with a startup screen (New Project wizard, Existing Project, or Continue Last Session) instead of silently booting into Draw mode, autosaves to recover from an unexpected close, and offers two bundled example projects via File → Sample Projects → Icon Font (Glyph Mode) / Shape Mode & Styles.
