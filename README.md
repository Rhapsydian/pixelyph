...
The most recent session reworked the Export modal into a checkbox multi-select (any combination bundles into one `.zip`, matching how font export already worked), added a Sprite Archive export and real control over raster export size (presets plus a custom scale or specific resolution, with an optional locked aspect ratio), gave new projects a few starter gradients and styles in the palette, and made every modal in the app follow the same cancel/confirm-with-a-divider footer convention. See "Draw mode" and "Animation" below for specifics.
