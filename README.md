...
Early development. Draw mode (both tiers, with frame-based animation), Glyph mode, project management (startup screen, new-project wizard, autosave recovery), font compilation, the Electron desktop shell, a full visual design pass (real app layout, a token-based dark theme, a viewport minimap, resizable panels), and a functional review of Palette/Layers/Style (a shared, exportable palette covering colors/gradients/saved styles with a dedicated Manage Swatches modal, per-layer thumbnails with per-frame visibility, a custom-built HSV color picker) are implemented — see "Features" below. The menu bar was also reworked: Export/Import Image/Reference Image moved from a crowded top-level menu and a side-panel tab into settings modals off the File menu, and a Help menu (About, GitHub link) and fullscreen toggle were added. The layer-offset X/Y inputs, layer groups/folders, and SVG pattern fills remain deliberately deferred or removed (see `BACKLOG.md`).

The most recent session closed four gaps between Draw mode and Glyph mode/save reliability: Glyph mode now supports symmetry/mirror drawing and the rectangle/ellipse Filled toggle, same as Draw mode; saved files (`.pixelyph` projects, exported fonts/images/zips) now reliably keep their intended extension even if a save dialog lets the filename get retyped without one; and `.pixelyph` project saves are roughly 8x smaller, since a grid cell only ever needs one bit (painted or not — color/style already lives on the layer) rather than a full byte. See "Draw mode," "Glyph mode," and "Project management" below for specifics.

A larger redesign — separating "layer" (identity/z-order) from "grid" (an independently-styled shape, closer to a vector-editor object) to fix a related sparsity cost in Simple-tier animations, and to let a single layer hold more than one shape — is underway on the `layer-frame-grid-redesign` branch. The full spec lives in [`docs/data-model.md`](./docs/data-model.md), covering exact data shapes, an Illustrator-style shape-selection model, the auto grow/shrink/merge algorithms, UI wireframes, and the save-file migration path. Design (Session 0), the model layer (Session 1 — `Layer`/`Frame`/`Grid` replace the old per-layer dense pixel buffer, plus a `.pixelyph` v3 save migration), export/compose (Session 2 — SVG, animated SVG, sprite sheet/archive, and GIF export all composite per-shape instead of per-layer), and the UI (Session 3 — the Layers panel shows each layer's Shapes as expandable sub-rows with a context-sensitive move/duplicate/merge/delete toolbar, the Style panel retargets to the active shape, and the pixel editor's cursor/paint-targeting and grid overlay read the active shape's own offset) are done — the branch is fully interactive again. `main` (and this live demo) still run the pre-redesign model described above until the final session (tests + hardening) lands and the branch merges.
...
- Layers panel: a per-layer live thumbnail, add/remove/reorder/duplicate/merge-down (merging keeps the bottom layer's style, matching Photoshop/Aseprite convention — move/duplicate/merge/delete act on whichever layer is currently active, from one toolbar rather than per-row buttons), a lock toggle, opacity, and an eyedropper that activates a layer instead of sampling a color (unambiguous once gradients exist)
