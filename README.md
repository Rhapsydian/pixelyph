...
A larger redesign — separating "layer" (identity/z-order) from "grid" (an independently-styled shape, closer to a vector-editor object) to fix a related sparsity cost in Simple-tier animations, and to let a single layer hold more than one shape — is underway on the `layer-frame-grid-redesign` branch. The full spec lives in [`docs/data-model.md`](./docs/data-model.md), covering exact data shapes, an Illustrator-style shape-selection model, the auto grow/shrink/merge algorithms, UI wireframes, and the save-file migration path. Design (Session 0), the model layer (Session 1 — `Layer`/`Frame`/`Grid` replace the old per-layer dense pixel buffer, plus a `.pixelyph` v3 save migration), and export/compose (Session 2 — SVG, animated SVG, sprite sheet/archive, and GIF export all composite per-shape instead of per-layer) are done. `main` (and this live demo) still run the pre-redesign model described above until the remaining sessions (UI, tests) land and merge — the branch is intentionally non-functional end-to-end in the meantime, since the model/export layers now expect shapes the UI doesn't create yet.
