...
The most recent session closed four gaps between Draw mode and Glyph mode/save reliability: Glyph mode now supports symmetry/mirror drawing and the rectangle/ellipse Filled toggle, same as Draw mode; saved files (`.pixelyph` projects, exported fonts/images/zips) now reliably keep their intended extension even if a save dialog lets the filename get retyped without one; and `.pixelyph` project saves are roughly 8x smaller, since a grid cell only ever needs one bit (painted or not — color/style already lives on the layer) rather than a full byte. See "Draw mode," "Glyph mode," and "Project management" below for specifics. A larger redesign — separating "layer" (identity/z-order) from "grid" (an independently-styled shape, closer to a vector-editor object) to fix a related sparsity cost in Simple-tier animations — is scoped and ready to start in `BACKLOG.md`.
