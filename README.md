...
A larger redesign — separating "layer" (identity/z-order) from "grid" (an independently-styled shape, closer to a vector-editor object) to fix a related sparsity cost in Simple-tier animations, and to let a single layer hold more than one shape — is underway on the `layer-frame-grid-redesign` branch. Session 0 (design) is done: the full spec lives in [`docs/data-model.md`](./docs/data-model.md), covering exact data shapes, an Illustrator-style shape-selection model, the auto grow/shrink/merge algorithms, UI wireframes, and the save-file migration path. `main` (and this live demo) still run the pre-redesign model described above until the remaining sessions (model layer, export, UI, tests) land and merge.
