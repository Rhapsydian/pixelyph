...
The most recent session shipped the Layer/Frame/Grid redesign: separating "layer" (identity/z-order) from "grid" — a "Shape" in the UI — an independently-styled object closer to a vector-editor path than the old dense per-layer pixel buffer. This fixes a real sparsity cost in Simple-tier animations (a color used in only 1 of 16 frames no longer allocates a full buffer in every frame) and lets a single Advanced-tier layer hold more than one shape, each with its own style/offset/size, resolving the old "layer groups" ask's actual premise. The Layers panel now shows each layer's Shapes as expandable sub-rows with their own add/reorder/duplicate/merge-down/delete toolbar; the Style panel, grid overlay, and cursor snapping all retarget to the active shape. Full spec, an Illustrator-style shape-selection model, and the save-file v3 migration: [`docs/data-model.md`](./docs/data-model.md). See "Draw mode" below for the Shapes panel and merge-behavior specifics.
...
- Layers panel: a per-layer live thumbnail, add/remove/reorder/duplicate/merge-down (a layer merge concatenates each frame's shapes — bottom's stay toward the back, top's toward the front — preserving every shape's own style, since each shape already carries its own; move/duplicate/merge/delete act on whichever layer is currently active, from one toolbar rather than per-row buttons), a lock toggle, opacity, and an eyedropper that activates a layer instead of sampling a color (unambiguous once gradients exist)
- Each layer expands to show its Shapes (one or more independently-styled, auto-cropped objects per frame — "Grid" internally, to avoid colliding with the pixel-snapping grid overlay's own name) as indented sub-rows with the same thumbnail/visibility/lock/name/opacity controls as a layer row, plus their own Add Shape/reorder/duplicate/merge-down/delete toolbar — merging two shapes *does* keep the bottom shape's style (Photoshop/Aseprite "flatten" convention), the mirror case from the layer merge above, since fusing two shapes into one can only keep a single style
...
- Selection scope toggle: marquee select/copy/cut can read from just the active shape (the default), just the active layer, or from whichever visible layer is topmost at each cell — paste always lands on the active layer either way
- Per-layer fill: solid, or a gradient (linear/radial, switchable in one Gradient Editor modal — a wide draggable stop bar, live-filled with the actual gradient as you drag, plus angle or center/radius controls depending on type) — solid and gradient-stop colors carry independent alpha (`#RRGGBBAA`), separate from stroke/effect alpha; any fill or a layer's whole style can be saved to the shared palette (an arrow-into-palette icon button) and reapplied to other layers
